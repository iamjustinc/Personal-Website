...
const handoffaiCaseStudyCopy: CaseStudyCopy = {
  tagline:
  ...
  pmInsight: {
    ...
    body: 'HandoffAI could have automatically generated and saved a handoff as soon as a user requested one. I chose to separate generation from persistence: every request defaults to preview, and a record is created only after the user explicitly asks to save it. Customer context can be incomplete or inaccurate, and a generated handoff may influence onboarding or implementation — a faster workflow is not useful if employees cannot trust what it writes into Salesforce.',
  },

  overviewCards: [
    {
      title: 'Product Thesis',
      body: 'A sales handoff is not just a summary. It is the point where responsibility and customer context move between teams, so the product needs to preserve what is known, expose what is missing, and create an accountable record the receiving team can retrieve later.',
    },
    {
      title: 'Product Bet',
      body: 'A structured first draft, paired with explicit review and approval, can improve handoff consistency without asking sales representatives to rewrite information that already exists in Salesforce.',
    },
  ],
  ...
  userCards: [
    ...
    },
    {
      ...
      body: 'When a customer is moving from Sales to another team, use the information already available in Salesforce to prepare a consistent handoff, show me what is missing, and let me approve the result before it becomes a permanent record.',
    },
  ],
  ...
  solutionCards: [
    {
      title: 'Six Structured Sections',
      body: 'Customer Overview, Current Stage, Missing Information, Suggested Next Action, Risks, and Questions to Ask — the same structure powers both product surfaces.',
    },
    {
      title: 'Agentforce: Preview → Save',
      body: '"Prepare a handoff" generates a preview and saves nothing. Only an explicit "Save this handoff" request creates one Sales_Handoff__c record and returns its ID — the Opportunity and Account stay unchanged either way.',
    },
    {
      title: 'Lightning Workspace',
      body: 'Generate, edit, validate, and save a handoff directly from the Opportunity record page. View the latest handoff, expand older history, copy the output, or mark it as reviewed.',
    },
  ],
  ...
  impactCards: [
    {
      title: 'Demonstrated Workflow Outcome',
      body: 'The intended Sales Handoff subagent was selected, the custom action launched successfully, preview ran with saving disabled, all six sections returned, no record was created during preview, and an explicit save created exactly one record with a returned ID while the source Opportunity stayed unchanged.',
    },
    {
      title: 'Engineering Validation',
      body: 'Apex and Lightning tests passed at the latest checkpoint, covering preview-versus-save behavior, structured persistence, large invocable batches, restricted-user access, invalid records, safe exception translation, record-navigation race conditions, and immediate history refresh.',
    },
  ],
  ...
  buildCards: [
    {
      title: 'Shared Service Architecture',
      body: 'Agentforce → GenerateSalesHandoffAction → HandoffService, and Lightning Web Component → HandoffController → HandoffService. The invocable action supports bulk requests without issuing one query or DML operation per record.',
    },
    {
      title: 'Secure, Fault-Tolerant Runtime',
      body: 'Sharing enforcement, user-mode queries and DML, field-access filtering, and a narrowly scoped permission set keep Opportunity and Account access read-only with no broad admin requirement. Invalid or inaccessible records return a safe message instead of a stack trace or fabricated content, and the Lightning component resets its state when the user navigates between Opportunities so one record’s handoff never leaks into another.',
    },
  ],
  ...
  reflectionCards: [
    {
      title: 'What I Cut',
      body: 'Automatic Opportunity or stage updates, task creation, Slack and email distribution, external generative AI, activity-history ingestion, multi-object support, packaging and marketplace publication, and production analytics — each would have expanded the surface area without proving the central workflow.',
    },
    {
      title: 'What Remained',
      body: 'The smallest complete trust loop: read permitted CRM context, generate six structured sections, expose missing information, let the user review, require explicit save authorization, and store a reusable Salesforce record.',
    },
    {
      title: "What's Next",
      body: 'Natural-language Opportunity lookup with disambiguation when names collide, a separate SaveApprovedSalesHandoff action that saves exactly the sections a user reviewed and edited, better navigation to the saved record, clean packaging and installation guidance, and validation with target users.',
    },
  ],

  screenshotCaption: 'HandoffAI Agentforce and Lightning workspace',
}
...
function getCaseStudyCopy(project: Project): CaseStudyCopy {
  if (project.slug === 'handoffai') return handoffaiCaseStudyCopy
...
export function ProjectCaseStudy({ project }: { project: Project }) {
  ...
  const isEnhancedCaseStudy = ['kestrel', 'harmoniq', 'handoffai'].includes(project.slug)
